...
class TranslationEngine {
    ...
    private func translateWithGoogle(text: String, settings: AppSettings, completion: @escaping (String?, String?) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            ...
            if let error = error {
                ...
                DispatchQueue.main.async { completion("⚠️ Erreur réseau — vérifiez votre connexion internet.", nil) }
                return
            }
            ...
            DispatchQueue.main.async { completion(translated, detectedLang) }
        }.resume()
    }
    ...
    private func translateWithGemini(text: String, settings: AppSettings, completion: @escaping (String?, String?) -> Void) {
        ...
        group.notify(queue: .main) {
            if let t = translatedText, t.hasPrefix("⚠️") {
                // Erreur Gemini → afficher directement, pas de fallback
                completion(t, nil)
            } else if translatedText == nil {
                // Gemini muet → fallback Google
